class Manage::ContractLinesController < Manage::ApplicationController
  ...
  def assign_or_create( quantity = (params[:quantity] || 1).to_i,
    ...
    contract ||= @user.contracts.new(inventory_pool: current_inventory_pool, status: @status) do |x|
  ...
  def create_contract_line(user, inventory_pool, status, record, quantity, start_date, end_date, purpose_id)
    ...
    # simply choose the first delegated user in order to pass contract validation. the delegated user has to be chosen again in the hand over process anyway
    contract_line.delegated_user = user.delegated_users.first if user.is_delegation
