class Borrow::ContractLinesController < Borrow::ApplicationController
  ...
  before_filter only: [:create, :change_time_range] do
    ...
    @target_contract = current_user.contracts.unsubmitted.find_or_initialize_by(inventory_pool_id: @inventory_pool.id, delegated_user_id: get_current_delegated_user.try(:id))
    @errors = []
  ...
  def create(model = current_user.models.borrowable.find(params[:model_id]),
    ...
    if @errors.empty? and (lines = model.add_to_contract(@target_contract, current_user, quantity, @start_date, @end_date)) and lines.all?(&:valid?)
