  ...
  def add_to_order
    ...
    if not unavailable_lines.empty? and params[:force_continue].blank?
      ...
      available_lines.each do |l|
        contract = current_user.contracts.unsubmitted.find_or_initialize_by(inventory_pool_id: l[:inventory_pool].id, delegated_user_id: get_current_delegated_user.try(:id))
        l[:model].add_to_contract(contract, current_user, l[:quantity], l[:start_date], l[:end_date])
      end
