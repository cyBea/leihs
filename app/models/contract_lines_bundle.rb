...
class ContractLinesBundle < ActiveRecord::Base
  ...
  LINE_CONDITIONS = -> (r){ where("(contract_lines.status IN ('#{:signed}', '#{:closed}') AND contract_lines.contract_id = ?)
                            ...
                            where("CASE contract_lines.status
                                    WHEN '#{:approved}' THEN contract_lines.start_date = :date
                                    WHEN '#{:signed}' THEN contract_lines.end_date = :date
                                    ELSE TRUE END", date: r.visit_date) }
  ...
  scope :search, lambda { |query|
                 ...
                 sql = uniq.
                     joins("INNER JOIN contract_lines ON
                            contract_lines_bundles.user_id = contract_lines.user_id
                            AND contract_lines_bundles.inventory_pool_id = contract_lines.inventory_pool_id
                            AND (
                                (contract_lines_bundles.status IN ('#{:signed}', '#{:closed}') AND contract_lines_bundles.id = contract_lines.contract_id)
                              OR
                                (contract_lines_bundles.status NOT IN ('#{:signed}', '#{:closed}') AND contract_lines_bundles.status = contract_lines.status)
                            )").
                     # joins("INNER JOIN contract_lines ON contract_lines.id IN contract_lines_bundles.contract_line_ids").
                     joins("INNER JOIN users ON users.id = contract_lines_bundles.user_id").
                     joins("LEFT JOIN contracts ON contract_lines_bundles.id = contracts.id AND contract_lines_bundles.status IN ('#{:signed}', '#{:closed}')").
