class User < ActiveRecord::Base
  ...
  #has_many :contract_lines_bundles
  has_many :contracts, class_name: "ContractLinesBundle" #, -> { uniq }, :through => :contract_lines
...
################################################
  ...
  def self.send_deadline_soon_reminder_to_everybody
    grouped_contract_lines = Visit.take_back.where(visit_date: Date.tomorrow).flat_map(&:lines).group_by { |vl| {inventory_pool: vl.inventory_pool, user_id: (vl.delegated_user_id || vl.user_id)} }
    grouped_contract_lines.each_pair do |k, contract_lines|
      user = User.find(k[:user_id])
