...
--
-- Temporary table structure for view `contract_lines_bundles`
--

DROP TABLE IF EXISTS `contract_lines_bundles`;
/*!50001 DROP VIEW IF EXISTS `contract_lines_bundles`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `contract_lines_bundles` (
  `id` varbinary(61),
  `user_id` int(11),
...
--
-- Final view structure for view `contract_lines_bundles`
--

/*!50001 DROP TABLE IF EXISTS `contract_lines_bundles`*/;
/*!50001 DROP VIEW IF EXISTS `contract_lines_bundles`*/;
...
/*!50001 VIEW `contract_lines_bundles` AS select ifnull(`cl`.`contract_id`,concat_ws('_',`cl`.`status`,`cl`.`user_id`,`cl`.`inventory_pool_id`)) AS `id`,`cl`.`user_id` AS `user_id`,`cl`.`inventory_pool_id` AS `inventory_pool_id`,`cl`.`status` AS `status`,if((`cl`.`status` = 'approved'),`cl`.`start_date`,if((`cl`.`status` = 'signed'),`cl`.`end_date`,NULL)) AS `visit_date`,sum(`cl`.`quantity`) AS `quantity`,if((sum(`groups`.`is_verification_required`) > 0),1,0) AS `verifiable_user`,(count(`partitions`.`id`) > 0) AS `verifiable_user_and_model`,max(`cl`.`created_at`) AS `created_at` from (((`contract_lines` `cl` left join `groups_users` on((`groups_users`.`user_id` = `cl`.`user_id`))) left join `groups` on(((`groups`.`id` = `groups_users`.`group_id`) and (`groups`.`inventory_pool_id` = `cl`.`inventory_pool_id`)))) left join `partitions` on(((`partitions`.`group_id` = `groups`.`id`) and (`partitions`.`model_id` = `cl`.`model_id`) and (`groups`.`is_verification_required` = 1)))) where (`cl`.`status` <> 'closed') group by ifnull(`cl`.`contract_id`,`cl`.`status`),`cl`.`user_id`,`cl`.`inventory_pool_id`,if((`cl`.`status` = 'signed'),`cl`.`end_date`,if((`cl`.`status` = 'approved'),`cl`.`start_date`,NULL)) order by if((`cl`.`status` = 'approved'),`cl`.`start_date`,if((`cl`.`status` = 'signed'),`cl`.`end_date`,NULL)) */;
...
-- Dump completed on 2015-03-24 21:11:10
