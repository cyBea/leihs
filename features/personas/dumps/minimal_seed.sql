...
--
-- Temporary table structure for view `contract_containers`
--

DROP TABLE IF EXISTS `contract_containers`;
/*!50001 DROP VIEW IF EXISTS `contract_containers`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `contract_containers` (
  `id` varbinary(61),
  `user_id` int(11),
  ...
  `created_at` datetime,
  `verifiable_user` int(1),
  `verifiable_user_and_model` int(1)
) ENGINE=MyISAM */;
...
--
-- Final view structure for view `contract_containers`
--

/*!50001 DROP TABLE IF EXISTS `contract_containers`*/;
/*!50001 DROP VIEW IF EXISTS `contract_containers`*/;
...
/*!50001 VIEW `contract_containers` AS select ifnull(`cl`.`contract_id`,concat_ws('_',`cl`.`status`,`cl`.`user_id`,`cl`.`inventory_pool_id`)) AS `id`,`cl`.`user_id` AS `user_id`,`cl`.`inventory_pool_id` AS `inventory_pool_id`,max(`cl`.`status`) AS `status`,`cl`.`delegated_user_id` AS `delegated_user_id`,max(`cl`.`created_at`) AS `created_at`,if((sum(`groups`.`is_verification_required`) > 0),1,0) AS `verifiable_user`,(count(`partitions`.`id`) > 0) AS `verifiable_user_and_model` from (((`contract_lines` `cl` left join `groups_users` on((`groups_users`.`user_id` = `cl`.`user_id`))) left join `groups` on(((`groups`.`id` = `groups_users`.`group_id`) and (`groups`.`inventory_pool_id` = `cl`.`inventory_pool_id`)))) left join `partitions` on(((`partitions`.`group_id` = `groups`.`id`) and (`partitions`.`model_id` = `cl`.`model_id`) and (`groups`.`is_verification_required` = 1)))) group by if(`cl`.`contract_id`,`cl`.`contract_id`,`cl`.`status`),`cl`.`user_id`,`cl`.`inventory_pool_id`,`cl`.`delegated_user_id` */;
...
/*!50001 SET collation_connection      = utf8_general_ci */;
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50001 VIEW `visits` AS select hex(concat_ws('_',if((`contract_lines`.`status` = 'approved'),`contract_lines`.`start_date`,`contract_lines`.`end_date`),`contract_lines`.`inventory_pool_id`,`contract_lines`.`user_id`,`contract_lines`.`status`)) AS `id`,`contract_lines`.`inventory_pool_id` AS `inventory_pool_id`,`contract_lines`.`user_id` AS `user_id`,`contract_lines`.`status` AS `status`,if((`contract_lines`.`status` = 'approved'),`contract_lines`.`start_date`,`contract_lines`.`end_date`) AS `date`,if((`contract_lines`.`status` = 'approved'),'hand_over','take_back') AS `action`,sum(`contract_lines`.`quantity`) AS `quantity` from `contract_lines` where ((`contract_lines`.`status` in ('approved','signed')) and isnull(`contract_lines`.`returned_date`)) group by `contract_lines`.`user_id`,`contract_lines`.`status`,if((`contract_lines`.`status` = 'approved'),`contract_lines`.`start_date`,`contract_lines`.`end_date`),`contract_lines`.`inventory_pool_id` order by if((`contract_lines`.`status` = 'approved'),`contract_lines`.`start_date`,`contract_lines`.`end_date`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2015-03-20  1:19:39
